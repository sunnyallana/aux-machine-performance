...
async function generatePDF(report) {
  return new Promise((resolve, reject) => {
    ...
    // OEE Highlight Box
    const oeeColor = getOEEColor(report.metrics.oee);
    ...
    doc.fontSize(7).fillColor('#6b7280').font('Helvetica').text(getOEEStatus(report.metrics.oee), leftCol + 80, currentY + 20);
    ...
    const ratioColor = getReliabilityColor(reliabilityRatio);
    const ratioStatus = getReliabilityStatus(reliabilityRatio);
    ...
    if (report.shiftData && report.shiftData.length > 0) {
      report.shiftData.slice(0, 3).forEach((shift, index) => {
        ...
        doc.roundedRect(rightCol + 180, rightY - 2, 45, 16, 8)
           .fillColor(getOEEColor(shiftOEE)).fill();
        ...
        doc.roundedRect(rightCol, rightY + 10, (qualityRate / 100) * barWidth, barHeight, 2)
           .fillColor(getQualityColor(qualityRate)).fill();
...
function getOEEColor(oee) {
  if (oee >= 85) return '#10b981'; // Excellent - Green
  if (oee >= 70) return '#f59e0b'; // Good - Amber  
  if (oee >= 50) return '#f97316'; // Fair - Orange
...
function getQualityColor(qualityRate) {
  if (qualityRate >= 95) return '#10b981'; // Excellent - Green
  if (qualityRate >= 85) return '#f59e0b'; // Good - Amber
  if (qualityRate >= 70) return '#f97316'; // Fair - Orange
  return '#ef4444'; // Poor - Red
}

function getReliabilityColor(ratio) {
  if (ratio >= 10) return '#10b981'; // Excellent - Green (High MTBF, Low MTTR)
  if (ratio >= 5) return '#f59e0b';   // Good - Amber
  if (ratio >= 2) return '#f97316';   // Fair - Orange
  return '#ef4444'; // Poor - Red (Low MTBF, High MTTR)
}

function getReliabilityStatus(ratio) {
  if (ratio >= 10) return 'EXCELLENT RELIABILITY';
  if (ratio >= 5) return 'GOOD RELIABILITY';
  if (ratio >= 2) return 'FAIR RELIABILITY';
